...
const removeUser = (id,setUsers) => {
    ...
    if (verification) {
        axios.post(`https://backend-online-betting.herokuapp.com/remove/user/${id}`)
...
const updateRank = (id,userType,setUsers) => {
    ...
    if(confirmUpdating) {
        ...
        axios.post(`https://backend-online-betting.herokuapp.com/update/rank/user/${id}`, { userType })
            .then(() => { setUsers([]) }).catch(error => console.log(error));
    };
};
